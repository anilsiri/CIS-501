...
public class CacheSampleTest {
    ...
    private static final String TRACE_FILE = "C://go-10M.trace.gz";
    ...
    public void setup() {
        ...
        IBranchTargetBuffer btb = new BranchTargetBuffer(18/*index bits*/);
...
*/
    ...
    public void testAlwaysTakenTrace() {
    	//for(INDEX_BITS=11; INDEX_BITS < 14; INDEX_BITS++){
        final IDirectionPredictor gshare = new DirPredGshare(18,18);
        final IBranchTargetBuffer bigBtb = new BranchTargetBuffer(18);
        UopIterator uiter = new UopIterator(TRACE_FILE, -1, new UopFactory());
        IInorderPipeline<Uop> pl = new InorderPipeline<>(new BranchPredictor(gshare, bigBtb), new Cache(INDEX_BITS, WAYS, BLOCK_BITS, HIT_LAT, CLEAN_MISS_LAT, DIRTY_MISS_LAT), new Cache(INDEX_BITS, WAYS, BLOCK_BITS, HIT_LAT, CLEAN_MISS_LAT, DIRTY_MISS_LAT));
        pl.run(uiter);
        System.out.println("Index bits is "+INDEX_BITS);
        System.out.println(pl.getInsns() / (double) pl.getCycles());
    	//}
       // assertEquals(0.571, pl.getInsns() / (double) pl.getCycles(), 0.01);
    } 
